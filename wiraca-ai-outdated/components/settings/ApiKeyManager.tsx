...
export default function ApiKeyManager({
  ...
  const [status, setStatus] = useState<"not_configured" | "active" | "error">(
    "not_configured",
  );
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center">
            <span
              className={`inline-block w-3 h-3 rounded-full mr-2 ${status === "active"
                ? "bg-green-500"
                : status === "error"
                  ? "bg-red-500"
                  : "bg-gray-400"
        ...
        {message && (
          <div
            className={`text-sm ${status === "active" ? "text-green-600" : "text-red-600"
              }`}
          >
            {message}
          </div>
        ...
        <button
          ...
          disabled={isLoading}
          className={`w-full px-4 py-2 rounded text-white ${isLoading
