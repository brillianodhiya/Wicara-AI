...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!outputType || !["summary", "action_items", "email"].includes(outputType)) {
      return NextResponse.json(
    ...
    switch (outputType) {
      case "summary":
        output = "This is a placeholder summary. The actual summary will be generated using Gemini API.";
        break;
      case "action_items":
        output = "- Action item 1\n- Action item 2\n- Action item 3";
        break;
      case "email":
        output = "Subject: Meeting Summary\n\nDear Team,\n\nThis is a placeholder email. The actual email will be generated using Gemini API.\n\nBest regards,\nWicara AI";
        break;
    }
